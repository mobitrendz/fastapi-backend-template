...
async def test_check_database_connection_failure(client: AsyncClient, mocker):
    ...
    mocker.patch("app.api.v1.endpoints.welcome.select") # Mock select to fail? No, select(1) just creates a statement.
    
    # Mocking the session itself might be better
    from sqlalchemy.exc import SQLAlchemyError
    mocker.patch("sqlalchemy.ext.asyncio.AsyncSession.execute", side_effect=SQLAlchemyError("Connection failed"))
