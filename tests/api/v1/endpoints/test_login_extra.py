...
async def test_recover_password_existing_user(client: AsyncClient, mocker):
    mocker.patch("app.api.v1.endpoints.login.security.render_email_template", return_value="<html>Reset</html>")
...
async def test_test_email(client: AsyncClient, mocker):
    ...
    mocker.patch("app.main.generate_test_email", return_value=EmailData(html_content="<html>Test</html>", subject="Test"))
    mock_send_email = mocker.patch("app.main.send_email")
    
    response = await client.post(
        "/test-email/?email_to=test@example.com"
    )
