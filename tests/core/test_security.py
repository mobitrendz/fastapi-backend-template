...
def test_generate_test_email(mocker):
    mocker.patch("app.core.security.render_email_template", return_value="<html>Test</html>")
...
def test_generate_reset_password_email(mocker):
    mocker.patch("app.core.security.render_email_template", return_value="<html>Reset</html>")
    email_to = "test@example.com"
    email = "user@example.com"
    token = "sometoken"
    email_data = security.generate_reset_password_email(email_to, email, token)
    assert email_data.subject == f"{settings.PROJECT_NAME} - Password recovery for user {email}"
